...
#+begin_src emacs-lisp :tangle ob-nasm.el
  (defun org-babel-execute:nasm (body params)
    "Execute a block of NASM code with org-babel."
    (let ((in-file (org-babel-temp-file "temp" ".s")))
      (with-temp-file in-file
        (if (string= (cdr (assq :snippet params)) "t")	
            (insert "global func\nsection .text\nfunc:"))
        (insert body))
      (org-babel-eval
       (format "nasm -felf64 %s -o temp.o"
               (org-babel-process-file-name in-file))
       "")
      (if (string= (cdr (assq :unwrapped params)) "t")	
          (org-babel-eval
           "ld temp.o -o temp" "")
        (org-babel-eval
         "gcc ~/.emacs.d/handler.c temp.o -o temp" ""))
      (org-babel-eval
       "./temp" "")))
